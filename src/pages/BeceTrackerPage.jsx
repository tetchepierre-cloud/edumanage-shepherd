...
export default function BeceTrackerPage() {
  ...
  const [allMocks, setAllMocks] = useState([]);        // tous les mocks
  const [subjectList, setSubjectList] = useState([]);   // toutes les matières JHS
  const [mockResults, setMockResults] = useState([]);   // notes mock de l'élève
  const [predictions, setPredictions] = useState({});   // { subject_id: avg }
  const [realResults, setRealResults] = useState({});   // { subject_id: score }
  ...
  const loadStudentData = async (studentId) => {
    // Trouver l'année académique à partir de la classe (JHS 3 → 2025/2026)
    // On simplifie : on utilise la variable d'état academicYear modifiable.

    // Charger les mock_results pour cet élève
    ...
    // Calculer la moyenne par matière sur tous les mocks disponibles
    const subjectScores = {};
    (mockData || []).forEach(r => {
    ...
    // Charger les résultats BECE réels déjà saisis
